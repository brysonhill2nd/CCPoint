...
class TennisGameState: ObservableObject, Identifiable, Hashable {
    let id = UUID()
    let gameType: GameType
    let settings: TennisSettings
    ...
    static func == (lhs: TennisGameState, rhs: TennisGameState) -> Bool {
        lhs.id == rhs.id
    }
    ...
    private func checkGameWinCondition() {
        ...
        if isInTiebreak {
            ...
            if settings.goldenPoint && atDeuce {
                ...
                // This will be handled on the next point scored
                return
    ...
    private func checkMatchWinCondition() {
        // Use TennisSettings for match format
